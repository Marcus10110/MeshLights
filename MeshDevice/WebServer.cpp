...
void WebServer::OnIndex( AsyncWebServerRequest* request )
{
    ...
    form.AddRatio( "mConnectionMode", "EXISTING_NETWORK", "Existing Wifi Network",
                   ( current_state->mConnectionMode == connection_mode_t::EXISTING_NETWORK ) );
    form.AddSubmitButton();
    ...
    // request->send(200, "text/html", "<form>Text to Broadcast<br><input type='text' name='BROADCAST'><br><br><input type='submit'
    // value='Submit'></form>");
}
...
void FormHelper::AddRatio( const String& name, const String& value, const String& contents, bool checked )
{
    ...
    mWorkingString +=
        "<input type='radio' name='" + name + "' value='" + value + "' " + ( checked ? "checked" : "" ) + ">" + contents + "<br>\n";
}
void FormHelper::AddNumberInput( const String& name, int min_value, int max_value, int existing_value )
{
    //<input type='number' name='quantity' min='1' max='5'>
    mWorkingString += "<input type='number' name='" + name + "' min='" + String( min_value ) + "' max='" + String( max_value ) +
                      "' value='" + String( existing_value ) + "'><br>\n";
}
