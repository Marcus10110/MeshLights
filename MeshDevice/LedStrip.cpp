...
LedStrip::LedStrip( uint16_t max_led_count, uint8_t led_pin, StateManager* state_manager, GetTimeMsCallback time_getter )
    : mStrip( max_led_count, led_pin ), mStateManager( state_manager ), mMaxLedCount( max_led_count ), mGetMeshTimeMs( time_getter )
{
}
void LedStrip::Init()
{
...
void LedStrip::RenderStateIndicator( uint32_t time_ms, uint16_t led_count )
{
    ...
    const uint32_t local_time_ms = time_ms % period_ms;
    const byte brightness = local_time_ms <= ( period_ms / 2 ) ? local_time_ms * 255 / ( period_ms / 2 )
                                                               : ( period_ms - local_time_ms ) * 255 / ( period_ms / 2 );
